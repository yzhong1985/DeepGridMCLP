...
@dataclass(frozen=True)  # frozen makes it immutable like NamedTuple
class AttentionModelFixed:
    ...
    __annotations__ = {'node_embeddings': torch.Tensor}
...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        # self.init_embed_f = nn.Linear(node_dim, embedding_dim)
    ...
    def forward(self, input, return_pi=False):
        ...
        if self.checkpoint_encoder and self.training:  # Only checkpoint if we need gradients
            embeddings, _ = checkpoint(self.embedder, self._init_embed(input['facilities']))
        else:
    ...
    def _init_embed(self, input):
        ...
        # loc = torch.concat((users, facilities), dim=1)
        return self.init_embed(facilities)
    ...
    def _inner(self, input, embeddings):
        ...
        while not (state.all_finished()):
            log_p, mask, last_hh = self._get_log_p_gru(fixed, state, normalize=True, last_hh=last_hh)
    ...
    def _get_log_p_gru(self, fixed, state, normalize=True, last_hh=None):
